...
from models.test import test_img, test_backdoor_mnist
import pandas as pd
...
if __name__ == '__main__':
    ...
    for iter in range(args.epochs):
        ...
        # backdoor attack train
        local = LocalUpdate(args=args, dataset=dataset_train, idxs=dict_users[-1])
        w_backdoor, loss_backdoor = local.backdoortrain_mnist(net=copy.deepcopy(net_glob).to(args.device), w_est_list=w_locals[:c])
        w_locals.append(copy.deepcopy(w_backdoor))
        loss_locals.append(copy.deepcopy(loss_backdoor))

        # update global weights
        # print(w_locals)
        w_glob = FedAvg(w_locals, mode='weight')
...
#%%
    ...
    net_glob.load_state_dict(w_backdoor)
    net_glob.eval()
    dataset_train_one_class = copy.deepcopy(dataset_train)
    dataset_test_one_class = copy.deepcopy(dataset_test)
    acc_backdoor_train, loss_backdoor_train = test_backdoor_mnist(net_glob, dataset_train_one_class, args, test_num = 6)
    # acc_backdoor_test, loss_backdoor_test = test_backdoor_img(net_glob, dataset_test_one_class, args, test_num = 6)
    print("The Training accuracy: {:.2f} of the test number {}".format(acc_backdoor_train, 6))
    # print("Backdoor testing accuracy: {:.2f}".format(acc_backdoor_test))

    dataset_train_one_class = copy.deepcopy(dataset_train)
    dataset_test_one_class = copy.deepcopy(dataset_test)
    acc_backdoor_train, loss_backdoor_train = test_backdoor_mnist(net_glob, dataset_train_one_class, args, test_num = 8)
    print("The Training accuracy: {:.2f} of the test number {}".format(acc_backdoor_train, 8))

    dataset_train_one_class = copy.deepcopy(dataset_train)
    dataset_test_one_class = copy.deepcopy(dataset_test)
    acc_backdoor_train, loss_backdoor_train = test_backdoor_mnist(net_glob, dataset_train_one_class, args, test_num = 9)
    print("The Training accuracy: {:.2f} of the test number {}".format(acc_backdoor_train, 9))